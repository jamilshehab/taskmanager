function agentFilter(agents) {
    return {
        ...
        filterAgents(e) {
            ...
            return (this.filteredAgents = this.agents.filter(
                (agent) =>
                    `${agent.name.toLowerCase()}`.includes(
                        e?.target?.value?.toLowerCase()
                    ) ||
                    `${agent.job.toLowerCase()}`.includes(
                        e?.target?.value?.toLowerCase()
                    )
            ));
        },
        addSelectedAgents(id) {
            const add_agents = this.filteredAgents.find(
                (agent) => agent.id === id
            );
        ...
        removeSelectedAgents(id) {
            this.selectedAgents = this.selectedAgents.filter(
                (agent) => agent.id !== id
            ); //so real time execution
            this.filteredAgents = this.selectedAgents.filter(
                (agent) => agent.id === id
            );
